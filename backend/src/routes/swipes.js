...
import { createSwipeSchema, setActiveSwipeSchema, validate } from '../validations/schemas.js';
...
export function createSwipesRouter(ctx) {
  ...
  router.post('/:messageId/swipes', requireAuth, validate(createSwipeSchema), (request, response) => {
    const messageId = request.params.messageId;
  ...
  router.put('/:messageId/swipes/active', requireAuth, validate(setActiveSwipeSchema), (request, response) => {
    const messageId = request.params.messageId;
