...
test('ChatView guards branch navigation after sidebar refresh awaits', () => {
  assert.match(
    chatViewScript,
    /async function createBranchFromMessage\(message\) \{\s*const branchConversationId = props\.route\.params\.id;\s*await handleBranchMessage\(message, branchConversationId, async \(branchId, isCurrentBranchAction, invalidateBranchAction\) => \{\s*if \(!isCurrentBranchAction\(\)\) \{\s*return;\s*\}\s*await loadSidebarData\(\);\s*if \(!isCurrentBranchAction\(\)\) \{\s*return;\s*\}\s*navigateFromBranchCreation\(branchId, invalidateBranchAction\);\s*\}\);\s*}/
  );
  assert.match(
    chatViewScript,
    /function navigateFromBranchCreation\(branchId, invalidateBranchAction\) \{\s*invalidateBranchAction\?\.\(\);\s*emit\('navigate', 'chat', \{ id: branchId \}\);\s*\}/
  );
});
