...
export function getActiveEncounter(database, userId, conversationId) {
  ...
  const row = database.prepare(
    "SELECT * FROM encounters WHERE conversation_id = ? AND status = 'active' ORDER BY created_at DESC, rowid DESC LIMIT 1"
  ).get(conversationId);
...
export function createEncounter(database, userId, conversationId, payload = {}, options = {}) {
  ...
  if (getActiveEncounter(database, userId, conversationId)) {
    return { ok: false, error: '已有进行中的遭遇' };
  }
  try {
    const protagonist = getProtagonist(database, userId, conversationId);
    const opponents = resolveOpponentMembers(database, userId, conversationId, payload);
    if (!opponents.length) return { ok: false, error: '遭遇需要至少一个 NPC' };
    const map = getTravelMap(database, userId, conversationId);
    const location = map?.currentNode || null;
    for (const member of opponents) {
      if (TERMINAL_MEMBER_STATUSES.has(member.status)) {
        return { ok: false, error: `终止状态 NPC 不能加入遭遇：${member.canonicalName}` };
      }
      const memberElsewhere = location && (
        (member.currentSceneNodeId && member.currentSceneNodeId !== location.id)
        || (!member.currentSceneNodeId
          && member.currentLocationLabel
          && member.currentLocationLabel !== location.name)
      );
      if (memberElsewhere) return { ok: false, error: `${member.canonicalName} 不在当前地点` };
    }
    return withSavepoint(database, 'sp_create_encounter', () => {
      const encounterId = newId();
      const timestamp = nowIso();
      const title = text(payload.title, 200)
        || `遭遇：${opponents.map((member) => member.canonicalName).join('、')}`;
      database.prepare(
        `INSERT INTO encounters (
           id, conversation_id, location_node_id, title, status, round_number,
           turn_index, outcome, source, created_at, updated_at
         ) VALUES (?, ?, ?, ?, 'active', 1, 0, '', ?, ?, ?)`
      ).run(
        encounterId,
        conversationId,
        location?.id || null,
        title,
        text(payload.source, 40) || 'player',
        timestamp,
        timestamp
      );
      const actors = [{
        type: 'player',
        memberId: protagonist.id,
        name: protagonist.canonicalName,
        hp: clampInteger(payload.playerHp, 1, 999, 20),
        defense: clampInteger(payload.playerDefense, 2, 40, 10),
      }];
      for (const member of opponents) {
        actors.push({ type: 'npc', memberId: member.id, name: member.canonicalName, hp: 10, defense: 10 });
      }
      const insertParticipant = database.prepare(
        `INSERT INTO encounter_participants (
           id, encounter_id, actor_type, actor_name, initiative, max_hp,
           current_hp, defense, status, conditions_json, created_at, updated_at, member_id
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', '[]', ?, ?, ?)`
      );
      for (let index = 0; index < actors.length; index += 1) {
        ...
        insertParticipant.run(
          ...
          actor.type,
          actor.name,
          rollD20(options.initiativeRolls?.[index]),
          actor.hp,
          actor.hp,
          actor.defense,
          timestamp,
      ...
      recordWorldEvent(database, userId, conversationId, {
        ...
        severity: 'warning',
        payload: { memberIds: opponents.map((member) => member.id) },
      });
...
export function performEncounterAction(database, userId, conversationId, encounterId, payload = {}, options = {}) {
  ...
  const actionType = ['attack', 'skill', 'defend', 'flee'].includes(payload.actionType)
    ? payload.actionType
    : '';
  ...
  return withSavepoint(database, 'sp_encounter_action', () => {
    ...
    if (actionType === 'attack' || actionType === 'skill') {
      target = encounter.participants.find((participant) => participant.id === payload.targetId);
      if (!target || target.status !== 'active' || target.actorType === actor.actorType) {
        return { ok: false, error: '目标无效' };
      }
      check = performSkillCheck(database, userId, conversationId, {
        actorName: actor.actorName,
        skill: text(payload.skill, 100) || (actionType === 'attack' ? '攻击' : '技能'),
        difficulty: target.defense,
        modifier: clampInteger(payload.modifier, -20, 20, 0),
        context: `遭遇：${encounter.title}`,
        source: payload.source || 'encounter',
      }, { roll: options.roll });
      if (check.outcome.includes('success')) {
        damage = rollDamage(options.damageRoll)
          + (check.outcome === 'critical_success' ? rollDamage(options.bonusDamageRoll) : 0);
        const nextHp = Math.max(0, target.currentHp - damage);
        database.prepare(
          "UPDATE encounter_participants SET current_hp = ?, status = CASE WHEN ? = 0 THEN 'defeated' ELSE status END, updated_at = ? WHERE id = ?"
        ).run(nextHp, nextHp, nowIso(), target.id);
        result.targetHp = nextHp;
      }
    } else if (actionType === 'defend') {
      database.prepare(
        'UPDATE encounter_participants SET defense = MIN(40, defense + 2), conditions_json = ?, updated_at = ? WHERE id = ?'
      ...
      if (result.escaped) {
        finishEncounter(
          ...
          userId,
          encounter,
          actor.actorType === 'player' ? 'escaped' : 'opponents_escaped',
          payload.source
        );
      }
    }
    ...
    database.prepare(
      `INSERT INTO encounter_actions (
         id, encounter_id, round_number, turn_index, actor_id, target_id,
         action_type, skill_check_id, damage, result_json, created_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      actionId,
      encounter.id,
    ...
    recordWorldEvent(database, userId, conversationId, {
      eventType: 'encounter.action',
      source: payload.source || 'player',
      title: `${actor.actorName}：${actionLabel(actionType)}${damage ? `（${damage} 伤害）` : ''}`,
      entityType: 'encounter',
...
export function endEncounter(database, userId, conversationId, encounterId, outcome = 'ended', source = 'player') {
  const encounter = readEncounter(database, encounterId);
  if (!encounter
    || encounter.conversationId !== conversationId
    || !hasConversationAccess(database, userId, conversationId)) {
    return null;
  }
...
function resolveOpponentMembers(database, userId, conversationId, payload) {
  const references = [];
  for (const memberId of normalizeReferences(payload.memberIds)) references.push({ memberId });
  if (!references.length) {
    for (const name of normalizeReferences(payload.memberNames || payload.npcNames)) references.push({ name });
  }
  const members = [];
  const seen = new Set();
  for (const reference of references) {
    const member = resolveCastMember(database, userId, conversationId, reference);
    if (member.memberType !== 'npc' || seen.has(member.id)) continue;
    seen.add(member.id);
    members.push(member);
  }
  return members;
...
function resolveEncounterOrAdvance(database, userId, encounterId, source) {
  const encounter = readEncounter(database, encounterId);
  const playerActive = encounter.participants.some(
    (participant) => participant.actorType === 'player' && participant.status === 'active'
  );
  const npcActive = encounter.participants.some(
    (participant) => participant.actorType === 'npc' && participant.status === 'active'
  );
  ...
  database.prepare(
    'UPDATE encounters SET round_number = ?, turn_index = ?, updated_at = ? WHERE id = ?'
  ).run(round, nextIndex, nowIso(), encounter.id);
}

function finishEncounter(database, userId, encounter, outcome, source) {
  database.prepare(
    "UPDATE encounters SET status = 'completed', outcome = ?, updated_at = ? WHERE id = ? AND status = 'active'"
  ).run(outcome, nowIso(), encounter.id);
  recordWorldEvent(database, userId, encounter.conversationId, {
    eventType: 'encounter.completed',
    source: source || 'encounter',
    title: `遭遇结束：${outcomeLabel(outcome)}`,
    entityType: 'encounter',
    entityId: encounter.id,
    severity: outcome === 'victory' ? 'success' : outcome === 'defeat' ? 'danger' : 'info',
  });
}
...
function readEncounter(database, id) {
  ...
  const participants = database.prepare(
    'SELECT * FROM encounter_participants WHERE encounter_id = ? ORDER BY initiative DESC, created_at, rowid'
  ).all(id).map(toParticipant);
  const actions = database.prepare(
    'SELECT * FROM encounter_actions WHERE encounter_id = ? ORDER BY created_at DESC, rowid DESC LIMIT 20'
  ).all(id).map((item) => ({
    id: item.id,
    ...
    skillCheckId: item.skill_check_id || '',
    damage: item.damage,
  ...
  return {
    ...
    source: row.source,
    participants,
    currentActor: participants[row.turn_index] || null,
    actions,
...
function normalizeReferences(value) {
  ...
  for (const item of Array.isArray(value) ? value : []) {
    const reference = text(item, 160);
    if (reference && !seen.has(reference) && result.length < 12) {
      seen.add(reference);
...
function rollD20(value) {
  return value === undefined ? crypto.randomInt(1, 21) : clampInteger(value, 1, 20, 10);
}

function rollDamage(value) {
  return value === undefined ? crypto.randomInt(1, 7) : clampInteger(value, 1, 6, 3);
}
...
function actionLabel(value) {
  return ({ attack: '攻击', skill: '技能', defend: '防御', flee: '脱离' })[value] || value;
}
